...
// Purchase Detail Modal Component (UNCHANGED)
...
const PurchaseHistoryTable = () => {
  const { setLoading, setError, purchases, setPurchases,currentPage, setCurrentPage,rowsPerPage } = useStore();
  ...
  useEffect(() => {
    const fetchPurchases = async () => {
      setLoading(true);
      try {
        const fetchedPurchases = await PurchaseApis.getAllPurchases();
        setPurchases(fetchedPurchases);
        setFilteredPurchases(fetchedPurchases);
      } catch (err) {
        toast.error(`Failed to load purchase history: ${err.message}`);
        setError(`Failed to load purchase history: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [searchQuery, purchases, startDate, endDate]);
  ...
  const currentPurchases = filteredPurchases.slice(indexOfFirstRow, indexOfLastRow);
  ...
  }, [currentPurchases]); // Depend on the sliced data for the current page
