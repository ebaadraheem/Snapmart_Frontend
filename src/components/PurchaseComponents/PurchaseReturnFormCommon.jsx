...
const PurchaseReturnFormCommon = ({
  titlePrefix,
        ...
        prevItems.map((item) => {
          if (item.productId === productId) {
            ...
            return {
              ...item,
              quantity: quantity,
              total: quantity * item.purchasePriceAtReturn,
            };
          }
          return item;
        })
  ...
  const handleRemoveReturnItem = useCallback((productId) => {
    ...
    setReturnCartItems((prevItems) => {
      ...
      // toast.success("Item removed from return list.");
      return updatedItems;
    });
  }, []);
  ...
  const handleCompleteReturn = useCallback(async () => {
    ...
    try {
      const returnData = {
        originalPurchase: supplier_purchase_details.purchase?._id || null,
        ...
        items: itemsToReturn.map((item) => ({
          ...
          purchasePriceAtReturn: item.purchasePriceAtReturn,
        })),
        totalReturnAmount: parseFloat(totalReturnAmount),
        ...additionalReturnData,
      };
                        ...
                        >
                          {modalItemsColumns.map((col) => (
                            <td
                              ...
                              className={`px-4 py-2 text-sm text-gray-900 text-${
                                col.align || "left"
                              }`}
                            >
      ...
      setReturnCartItems((prevItems) => {
        ...
        if (existingItemIndex > -1) {
          ...
          updatedItems[existingItemIndex] = {
            ...
            purchasePriceAtReturn: purchasePrice,
            total: newTotalQuantity * purchasePrice,
          };
          ...
          const newItem = {
            ...
            purchasePriceAtReturn: purchasePrice,
            total: newItemInitialQuantity * purchasePrice,
          };
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {returnCartItems.map((item) => (
                  <tr key={item.productId} className="text-sm max-sm:text-xs">
                    ...
                    <td className="px-4 py-2 whitespace-nowrap  text-gray-700">
                      {item.purchasePriceAtReturn.toFixed(2)}
                    </td>
...
export default PurchaseReturnFormCommon;
