...
// Sales Detail Modal Component (UNCHANGED)
const SalesDetailModal = ({ sale, onClose }) => {
      ...
      >
        ...
        <div className="bg-white max-h-[90vh] overflow-y-auto border-t-4 border-blue-900/90 text-blue-900/80 rounded-lg shadow-xl p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-blue-900/90">Sale Details</h2>
          </div>

          <div className="space-y-3 text-sm">
            <Detail label="Invoice #" value={sale.receiptId} />
            <Detail label="Customer" value={`${sale.customerName || "N/A"}`} />
            <Detail
              label="Sale Date"
              value={new Date(sale.date).toLocaleDateString()}
            />
            <Detail label="Total Amount" value={`${sale.totalAmount || "0"}`} />
            <Detail label="Paid Amount" value={`${sale.paidAmount || "0"}`} />
            <Detail label="Discount" value={`${sale.discount || "0"}`} />
            {sale.profit >= 0 ? (
              <Detail label="Profit" value={`${sale.profit || "0"}`} />
            ) : (
              <Detail label="Loss" value={`${Math.abs(sale.profit) || "0"}`} />
            )}
          </div>
...
const SalesList = () => {
  const { setLoading, setError, sales, setSales,currentPage, setCurrentPage,rowsPerPage } = useStore();
  ...
  useEffect(() => {
    const fetchsales = async () => {
      setLoading(true);
      try {
        const fetchedSales = await SaleApis.getAllSales("");
        setSales(fetchedSales);
        setFilteredSales(fetchedSales); // Initially, all sales are shown
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const filtered = sales.filter((sale) => {
      const matchesText =
        !searchQuery ||
        (sale.receiptId || "").toLowerCase().includes(lowercasedQuery) ||
  ...
  // Memoize the table rows based on the current page's data
  ...
  }, [currentSales]); // Depend on the sliced data for the current page



  return (
    <div className="p-6 bg-white rounded-lg shadow-md flex flex-col h-full">
      <h2 className="sm:text-2xl text-lg font-bold text-blue-900/90 mb-6 border-b pb-3 flex items-center sm:gap-1">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 mb-4">
        ...
        {/* Date Filters */}
        <div className="flex flex-wrap items-center gap-2 text-sm max-sm:text-xs">
